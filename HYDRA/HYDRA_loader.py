...
def HYDRA(win_size, stride=1, K=2, include_all_windows=True, debug=False,
                 ...
                 ann_threads=1, mode='approx',random_state=42):
    if mode == 'exact':
        return HYDRAExact(win_size, stride=stride, K=K, include_all_windows=include_all_windows, debug=debug, random_state=random_state)
    elif mode == 'approx':
        return HYDRAApprox(win_size, stride=stride, K=K, include_all_windows=include_all_windows, debug=debug,
                           ...
                           hnsw_ef=hnsw_ef, ann_backend=ann_backend, random_state=random_state)
