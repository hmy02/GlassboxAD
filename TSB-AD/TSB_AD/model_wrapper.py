...
Unsupervise_AD_Pool = ['HYDRA', 'FFT', 'SR', 'NORMA', 'Series2Graph', 'Sub_IForest', 'IForest', 'LOF', 'Sub_LOF', 'POLY', 'MatrixProfile', 'Sub_PCA', 'PCA', 'HBOS',
...
def run_HYDRA(data, periodicity=None, winsize=None, K=2):
    from .models.HYDRA import HYDRA
    if periodicity!=None:
        slidingWindow = find_length_rank(data, rank=periodicity)
    else:
        slidingWindow = winsize
    model = HYDRA(win_size=slidingWindow, stride=1, K=K, include_all_windows=True, debug=True)
    ts_scores, win_scores = model.compress_and_score_multi(data)
    ens_score = model.ensemble_scores()
    return ens_score
